...
export default function EditInvoicePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [billingType, setBillingType] = useState('full_work');
    const [workflowStepInstanceId, setWorkflowStepInstanceId] = useState('');
    ...
    useEffect(() => {
        const fetchInitial = async () => {
            ...
            await Promise.all([fetchClientsData(), fetchProfilesData()]);
            ...
            try {
                ...
                if (data.success && data.data) {
                    ...
                    setWorkId(String(inv.work_id || ''));
                    setProfileId(String(inv.business_profile_id || ''));
                    setBillingType(inv.billing_type || 'full_work');
                    setWorkflowStepInstanceId(String(inv.workflow_step_instance_id || ''));
                    if (inv.invoice_date) setInvoiceDate(inv.invoice_date.split('T')[0]);
    ...
    useEffect(() => {
        ...
        const fetchAutofillData = async () => {
            ...
            try {
                ...
                if (!autofillRes.ok) {
                    setAutofillError(`Autofill endpoint unavailable (${autofillRes.status})`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload = {
                ...
                workflow_step_instance_id: billingType === 'workflow_step' ? workflowStepInstanceId : null,
            ...
            if (data.success) {
                toast({
                    title: "Success",
                    description: "Internal bill updated successfully",
                });
                router.push(`/dashboard/accounts/billing/${id}`);
            } else {
                ...
                <div className="w-full flex items-start gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="text-3xl font-bold tracking-tight text-slate-900">Edit Bill</h1>
                            <Badge variant="secondary" className="bg-slate-100 text-slate-700 font-medium">Draft</Badge>
                        </div>
                        <p className="text-sm text-slate-500 mt-1">Update internal draft bill.</p>
                                ...
                                <CardContent className="p-0 overflow-hidden">
                                    {autofillError && (
                                        <div className="px-6 pt-4 pb-2">
                                            <div className="flex items-start gap-2 text-sm text-amber-700 bg-amber-50 p-3 rounded-md border border-amber-200/50">
